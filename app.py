"""
🛡️ Buddywise Safety Intelligence Command Center
A comprehensive workplace safety analytics platform mirroring Buddywise's product:
PPE Detection, Housekeeping, Area Control, Vehicle Control, Person Down, Crane Safety

Features: Zone Heatmaps, Logbook, Notification Center, Predictive Analytics, Executive Dashboard
...
# ─── Dark Theme Colour Palette (Buddywise Brand-Aligned) ─────────────────────
BG_COLOR = "#0B1120"
CARD_COLOR = "#151E32"
CARD_HOVER = "#1E293B"
BORDER_COLOR = "#2D3A4F"
...
<style>
    ...
    .risk-critical {{ background: {CRITICAL_COLOR}15; border-color: {CRITICAL_COLOR}; color: {CRITICAL_COLOR}; }}
    .risk-high {{ background: {HIGH_COLOR}15; border-color: {HIGH_COLOR}; color: {HIGH_COLOR}; }}
    .risk-medium {{ background: {MEDIUM_COLOR}15; border-color: {MEDIUM_COLOR}; color: {MEDIUM_COLOR}; }}
    .risk-low {{ background: {LOW_COLOR}15; border-color: {LOW_COLOR}; color: {LOW_COLOR}; }}
...
<div style="padding: 8px 0 16px 0;">
    <h2 style="color:{TEXT_COLOR}; margin:0; font-size:20px; font-weight:800;">🛡️ Buddywise</h2>
    <p style="color:{MUTED_TEXT}; margin:4px 0 0 0; font-size:12px;">Safety Intelligence Command Center</p>
</div>
""", unsafe_allow_html=True)
...
# Filters
st.sidebar.markdown(f"<h4 style='color:{TEXT_COLOR}; font-size:14px;'>🎛️ Filters</h4>", unsafe_allow_html=True)
...
selected_scenarios = st.sidebar.multiselect(
    ...
    default=list(RISK_SCENARIOS.keys())
)
...
# ─── Helper Functions ──────────────────────────────────────────────────────
def create_kpi_card(title, value, subtitle="", color=TEXT_COLOR, delta=None):
    delta_html = f'<div class="metric-delta" style="color:{LOW_COLOR if delta and delta.startswith("-") else CRITICAL_COLOR if delta and not delta.startswith("-") else MUTED_TEXT};">{delta}</div>' if delta else ""
    st.markdown(f"""
...
with tab1:
    ...
    <div style="margin-bottom:24px;">
        <h2 style="color:{TEXT_COLOR}; margin:0; font-size:28px; font-weight:800;">Executive Command Center</h2>
        <p style="color:{MUTED_TEXT}; margin:6px 0 0 0; font-size:14px;">Real-time safety intelligence across {len(selected_sites)} industrial sites · {len(filtered_df):,} alerts analyzed</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        create_kpi_card("Total Alerts", f"{total:,}", "All scenarios")

    with col2:
        critical = len(filtered_df[filtered_df['severity'] == 'CRITICAL'])
        crit_pct = (critical / total * 100) if total > 0 else 0
        create_kpi_card("Critical", f"{critical}", f"{crit_pct:.1f}% of total", CRITICAL_COLOR)

    with col3:
        unresp_crit = len(filtered_df[(filtered_df['severity'] == 'CRITICAL') & (filtered_df['responded'] == False)])
        create_kpi_card("Unresponded Critical", f"{unresp_crit}", "Requires escalation", CRITICAL_COLOR if unresp_crit > 0 else TEXT_COLOR)

    with col4:
        fp_rate = filtered_df['is_false_positive'].mean() * 100
        create_kpi_card("False Positive Rate", f"{fp_rate:.1f}%", "AI model quality", MEDIUM_COLOR if fp_rate > 8 else LOW_COLOR)

    with col5:
        avg_resp = filtered_df[filtered_df['responded'] == True]['response_time_mins'].mean()
        create_kpi_card("Avg Response", f"{avg_resp:.1f}m", "All severity levels", ACCENT_COLOR)

    with col6:
        resolved = filtered_df['resolved'].sum()
        res_rate = (resolved / total * 100) if total > 0 else 0
        create_kpi_card("Resolution Rate", f"{res_rate:.1f}%", f"{resolved} resolved", LOW_COLOR)
    ...
    with col1:
        st.markdown(f"<h4 style='color:{TEXT_COLOR}; font-size:16px;'>📈 Daily Alert Volume by Scenario</h4>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"<h4 style='color:{TEXT_COLOR}; font-size:16px;'>🎯 Scenario Breakdown</h4>", unsafe_allow_html=True)
        ...
        fig = go.Figure(data=[go.Pie(
            ...
            pull=[0.05 if s == 'Person Down' else 0 for s in scenario_counts.index]
        )])
        fig.update_layout(
            plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
    ...
    with col1:
        st.markdown(f"<h4 style='color:{TEXT_COLOR}; font-size:16px;'>⚠️ Severity Distribution</h4>", unsafe_allow_html=True)
        ...
        fig = go.Figure(go.Bar(
            ...
            marker_color=colors, text=sev_counts.values, textposition='outside',
    ...
    with col2:
        st.markdown(f"<h4 style='color:{TEXT_COLOR}; font-size:16px;'>🕐 Alerts by Hour</h4>", unsafe_allow_html=True)
        ...
        fig.add_vrect(x0=22, x1=6, fillcolor=CRITICAL_COLOR, opacity=0.12, line_width=0, 
                     annotation_text="Night Shift", annotation_position="top left",
                     annotation_font_color=CRITICAL_COLOR)
        fig.update_layout(plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
                         xaxis_title="Hour", yaxis_title="Count", margin=dict(l=20, r=20, t=60, b=20), height=300)
        st.plotly_chart(fig, use_container_width=True)

    with col3:
        st.markdown(f"<h4 style='color:{TEXT_COLOR}; font-size:16px;'>🏭 Alerts by Site</h4>", unsafe_allow_html=True)
        site_counts = filtered_df['site'].value_counts()
        fig = go.Figure(go.Bar(
            y=site_counts.index, x=site_counts.values, orientation='h',
            marker=dict(color=site_counts.values, colorscale=[[0, LOW_COLOR], [0.5, MEDIUM_COLOR], [1, CRITICAL_COLOR]],
                       showscale=False), text=site_counts.values, textposition='outside'
        ))
        fig.update_layout(plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
                         margin=dict(l=20, r=40, t=40, b=20), height=300, yaxis=dict(tickfont=dict(size=10)))
        st.plotly_chart(fig, use_container_width=True)

    # Critical Insights
    st.divider()
    st.markdown(f"<h4 style='color:{TEXT_COLOR}; font-size:16px;'>🚨 Critical Insights</h4>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"""
        <div class="insight-card {'insight-critical' if unresp_crit > 0 else ''}">
            <b style="color:{CRITICAL_COLOR};">⚠️ Unresponded Critical Alerts</b><br>
            <span style="color:{TEXT_COLOR};">{unresp_crit} CRITICAL alerts without response. 
    ...
    with col2:
        ...
        <div class="insight-card">
            ...
            <span style="color:{TEXT_COLOR};">Night shift critical rate: <b>{night_crit_rate:.1f}%</b> vs day: <b>{day_crit_rate:.1f}%</b>. 
    ...
    with col3:
        ...
        <div class="insight-card">
            ...
            <span style="color:{TEXT_COLOR};">Untrained workers: <b>{untrained_crit_rate:.1f}%</b> critical rate vs trained: <b>{trained_crit_rate:.1f}%</b>. 
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 2 — ZONE HEATMAPS (Buddywise Core Feature)
# ═══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown(f"""
    <div style="margin-bottom:24px;">
        <h2 style="color:{TEXT_COLOR}; margin:0; font-size:28px; font-weight:800;">🔥 Zone Heatmaps</h2>
        <p style="color:{MUTED_TEXT}; margin:6px 0 0 0; font-size:14px;">Identify safety hotspots across zones — Buddywise's signature analytics feature</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if selected_heatmap_site:
        ...
        with col1:
            st.markdown(f"<h4 style='color:{TEXT_COLOR};'>Zone × Severity Heatmap</h4>", unsafe_allow_html=True)
            ...
            fig.update_layout(
                plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
                xaxis_title="Severity", yaxis_title="Zone",
                margin=dict(l=20, r=80, t=40, b=20), height=450
            )
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            st.markdown(f"<h4 style='color:{TEXT_COLOR};'>Zone Risk Summary</h4>", unsafe_allow_html=True)
            ...
            for _, row in zone_risk.head(5).iterrows():
                ...
                st.markdown(f"""
                <div style="background:{CARD_COLOR}; border:1px solid {BORDER_COLOR}; border-radius:10px; padding:12px; margin:8px 0;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span style="color:{TEXT_COLOR}; font-weight:600; font-size:13px;">{row['zone']}</span>
                        <span style="color:{risk_color}; font-weight:700; font-size:14px;">{row['critical_pct']:.1f}% CRIT</span>
                    </div>
                    <div style="color:{MUTED_TEXT}; font-size:11px; margin-top:4px;">{row['total']} alerts · {row['avg_response']:.1f}m avg response</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            if high_risk_in_site:
                st.markdown(f"""
                <div style="background:{WARNING_BG}; border:1px solid {CRITICAL_COLOR}; border-radius:10px; padding:12px; margin-top:12px;">
                    <span style="color:{CRITICAL_COLOR}; font-weight:700; font-size:13px;">⚠️ High-Risk Zones Active</span><br>
                    <span style="color:{TEXT_COLOR}; font-size:12px;">{', '.join(high_risk_in_site)} require enhanced monitoring per Buddywise safety protocols.</span>
                </div>
                """, unsafe_allow_html=True)

    st.divider()

    # Cross-site heatmap comparison
    st.markdown(f"<h4 style='color:{TEXT_COLOR};'>Cross-Site Critical Alert Density</h4>", unsafe_allow_html=True)
    ...
    fig.update_layout(
        plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
        xaxis_title="Zone", yaxis_title="Site",
        margin=dict(l=20, r=80, t=40, b=20), height=350
    )
    st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    st.markdown(f"""
    <div style="margin-bottom:24px;">
        <h2 style="color:{TEXT_COLOR}; margin:0; font-size:28px; font-weight:800;">👷 Worker Intelligence</h2>
        <p style="color:{MUTED_TEXT}; margin:6px 0 0 0; font-size:14px;">Risk profiling, training impact, and behavioral analytics</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"<h4 style='color:{TEXT_COLOR};'>Top 15 Highest-Risk Workers</h4>", unsafe_allow_html=True)
        top_risk = filtered_workers.nlargest(15, 'risk_score')

        fig = go.Figure(go.Bar(
            x=top_risk['risk_score'],
            y=top_risk['worker_id'],
            orientation='h',
            marker=dict(
                color=top_risk['risk_score'],
                colorscale=[[0, LOW_COLOR], [0.4, MEDIUM_COLOR], [0.7, HIGH_COLOR], [1, CRITICAL_COLOR]]
            ),
        ...
        fig.update_layout(
            plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
            xaxis_title="Risk Score", yaxis_title="Worker ID",
            margin=dict(l=20, r=60, t=40, b=20), height=450
        )
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        st.markdown(f"<h4 style='color:{TEXT_COLOR};'>Department Risk Analysis</h4>", unsafe_allow_html=True)
        ...
        fig.add_trace(go.Bar(
            x=dept_stats['department'], y=dept_stats['avg_risk'],
            name='Avg Risk Score', marker_color=ACCENT_COLOR,
            text=dept_stats['avg_risk'].round(1), textposition='outside'
        ), secondary_y=False)
        fig.add_trace(go.Scatter(
            x=dept_stats['department'], y=dept_stats['headcount'],
            name='Headcount', mode='lines+markers',
            line=dict(color=MUTED_TEXT, width=2), marker=dict(size=8)
        ), secondary_y=True)
        fig.update_layout(
            plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
            margin=dict(l=20, r=60, t=40, b=20), height=450,
    ...
    # Training Impact Deep Dive
    st.markdown(f"<h4 style='color:{TEXT_COLOR};'>📚 Training Impact Analysis</h4>", unsafe_allow_html=True)
    ...
    for col, (metric, title, subtitle) in zip([col1, col2, col3, col4], metrics):
        with col:
            ...
            for i, row in train_df.iterrows():
                fig.add_trace(go.Bar(
                    x=[row['status']], y=[row[metric]],
                    marker_color=colors[i], text=[f"{row[metric]:.2f}"],
                    textposition='outside', textfont=dict(color=TEXT_COLOR, size=14)
                ))
            fig.update_layout(
                plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
                showlegend=False, yaxis_title=title, margin=dict(l=20, r=20, t=20, b=20), height=280
            )
            st.plotly_chart(fig, use_container_width=True)
            st.markdown(f"<p style='text-align:center; color:{MUTED_TEXT}; font-size:11px;'>{subtitle}</p>", unsafe_allow_html=True)

    # Training ROI Callout
    ...
    <div style="background: linear-gradient(90deg, {ACCENT_COLOR}20, {LOW_COLOR}20); border:1px solid {ACCENT_COLOR}; border-radius:16px; padding:24px; margin-top:20px; text-align:center;">
        ...
        <p style="color:{TEXT_COLOR}; margin:8px 0 0 0; font-size:15px;">
            ...
            At an estimated €50,000 per serious injury (DGUV data), training investment pays for itself in <b>3 weeks</b>.
        </p>
    </div>
    """, unsafe_allow_html=True)
...
with tab4:
    st.markdown(f"""
    <div style="margin-bottom:24px;">
        <h2 style="color:{TEXT_COLOR}; margin:0; font-size:28px; font-weight:800;">🔮 Predictive Risk Engine</h2>
        <p style="color:{MUTED_TEXT}; margin:6px 0 0 0; font-size:14px;">Simulate scenarios and predict risk scores before incidents occur</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"<h4 style='color:{TEXT_COLOR};'>Scenario Configuration</h4>", unsafe_allow_html=True)
        ...
        pred_violation = st.selectbox("⚠️ Violation Type", options=RISK_SCENARIOS[pred_scenario]['violations'], key="pred_violation")
        ...
        if risk_score >= 85:
            ...
            advice = "🛑 IMMEDIATE ACTION: Stop all work. Full safety lockdown. Escalate to site safety officer + emergency services on standby."
        elif risk_score >= 65:
            risk_level = "HIGH"; risk_class = "risk-high"
            advice = "⚠️ HIGH PRIORITY: Supervisor must verify before work proceeds. Enhanced PPE check. Buddy system mandatory."
        elif risk_score >= 40:
    ...
    with col2:
        st.markdown(f"<h4 style='color:{TEXT_COLOR};'>Risk Assessment Result</h4>", unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div style="background:{CARD_COLOR}; border:1px solid {BORDER_COLOR}; border-radius:12px; padding:20px; margin-top:16px;">
            <b style="color:{ACCENT_COLOR}; font-size:14px;">🎯 Recommended Action:</b><br>
            <p style="color:{TEXT_COLOR}; margin:8px 0 0 0; font-size:14px; line-height:1.6;">{advice}</p>
        </div>
        """, unsafe_allow_html=True)

        # Score breakdown
        st.markdown(f"<h5 style='color:{TEXT_COLOR}; margin-top:20px;'>📊 Score Breakdown</h5>", unsafe_allow_html=True)
        ...
        # What-if comparison
        st.markdown(f"<h5 style='color:{TEXT_COLOR}; margin-top:20px;'>🔬 What-If: Training Impact</h5>", unsafe_allow_html=True)

        # Recalculate with training
        ...
        with col_a:
            st.markdown(f"""
            <div style="background:{WARNING_BG}; border:1px solid {CRITICAL_COLOR}; border-radius:10px; padding:16px; text-align:center;">
                <div style="color:{MUTED_TEXT}; font-size:12px;">WITHOUT TRAINING</div>
                <div style="color:{CRITICAL_COLOR}; font-size:32px; font-weight:800;">{risk_score}</div>
                <div style="color:{TEXT_COLOR}; font-size:12px;">{risk_level}</div>
            </div>
            """, unsafe_allow_html=True)
        with col_b:
            ...
            st.markdown(f"""
            <div style="background:{LOW_COLOR}15; border:1px solid {LOW_COLOR}; border-radius:10px; padding:16px; text-align:center;">
                <div style="color:{MUTED_TEXT}; font-size:12px;">WITH TRAINING</div>
                <div style="color:{trained_color}; font-size:32px; font-weight:800;">{trained_score}</div>
                <div style="color:{TEXT_COLOR}; font-size:12px;">{trained_level}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    # Historical drivers
    st.markdown(f"<h4 style='color:{TEXT_COLOR};'>📈 Historical Drivers of Critical Alerts</h4>", unsafe_allow_html=True)

    crit_df = filtered_df[filtered_df['severity'] == 'CRITICAL']
    col1, col2, col3 = st.columns(3)

    with col1:
        crit_scenario = crit_df['scenario_type'].value_counts().head(5)
        fig = go.Figure(go.Bar(x=crit_scenario.values, y=crit_scenario.index, orientation='h',
                              marker_color=CRITICAL_COLOR, text=crit_scenario.values, textposition='outside'))
        fig.update_layout(plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
                         margin=dict(l=20, r=40, t=20, b=20), height=280, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        crit_shift = crit_df['shift'].value_counts()
        fig = go.Figure(go.Pie(labels=crit_shift.index, values=crit_shift.values,
                              marker_colors=[MEDIUM_COLOR, HIGH_COLOR, CRITICAL_COLOR],
                              textinfo='label+percent', textfont=dict(color=TEXT_COLOR)))
        fig.update_layout(plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
    ...
    with col3:
        ...
        fig.update_layout(plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
                         xaxis_title="Hour", yaxis_title="Critical Count",
                         margin=dict(l=20, r=20, t=20, b=20), height=280, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)


# ═══════════════════════════════════════════════════════════════════════════════
# TAB 5 — LOGBOOK & RESOLVE (Buddywise Core Feature)
# ═══════════════════════════════════════════════════════════════════════════════
with tab5:
    st.markdown(f"""
    <div style="margin-bottom:24px;">
        <h2 style="color:{TEXT_COLOR}; margin:0; font-size:28px; font-weight:800;">📋 Logbook & Resolve</h2>
        <p style="color:{MUTED_TEXT}; margin:6px 0 0 0; font-size:14px;">Review, assign, comment, and resolve safety events — just like Buddywise's logbook</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Format for display
    display_cols = ['alert_id', 'timestamp', 'site', 'zone', 'scenario_type', 'violation_type', 
                   'severity', 'shift', 'worker_id', 'responded', 'resolved', 'assigned_to', 'comments']
    ...
    display_formatted['responded'] = display_formatted['responded'].map({True: '✅', False: '❌'})
    display_formatted['resolved'] = display_formatted['resolved'].map({True: '✅', False: '⏳'})
    ...
    with col2:
        ...
        st.markdown(f"<p style='color:{MUTED_TEXT}; font-size:13px; text-align:center; padding-top:8px;'>{unresolved_count} unresolved events requiring attention</p>", unsafe_allow_html=True)
    with col3:
    ...
    st.divider()
    st.markdown(f"<h4 style='color:{TEXT_COLOR};'>📝 Quick Assignment Simulation</h4>", unsafe_allow_html=True)
    ...
    if len(unassigned) > 0:
        for _, alert in unassigned.iterrows():
            ...
            with col1:
                st.markdown(f"""
                <div style="background:{CARD_COLOR}; border:1px solid {BORDER_COLOR}; border-radius:8px; padding:10px;">
                    <span style="color:{SEVERITY_COLORS[alert['severity']]}; font-weight:700;">{alert['severity']}</span>
                    <span style="color:{TEXT_COLOR}; font-size:13px;"> · {alert['violation_type']} · {alert['zone']}</span>
                    <span style="color:{MUTED_TEXT}; font-size:11px; display:block; margin-top:2px;">{alert['timestamp'].strftime('%Y-%m-%d %H:%M')} · {alert['site']}</span>
                </div>
                """, unsafe_allow_html=True)
            with col2:
                assignee = st.selectbox(f"Assign {alert['alert_id']}", 
                                       options=['Safety Officer', 'Supervisor', 'Site Manager', 'Engineering'],
                                       key=f"assign_{alert['alert_id']}", label_visibility="collapsed")
            with col3:
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 6 — NOTIFICATION CENTER (Buddywise Core Feature)
# ═══════════════════════════════════════════════════════════════════════════════
with tab6:
    st.markdown(f"""
    <div style="margin-bottom:24px;">
        <h2 style="color:{TEXT_COLOR}; margin:0; font-size:28px; font-weight:800;">🔔 Smart Notification Center</h2>
        <p style="color:{MUTED_TEXT}; margin:6px 0 0 0; font-size:14px;">Configure smart thresholds and notification rules — SMS, Email, IoT, Dashboard</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"<h4 style='color:{TEXT_COLOR};'>⚙️ Notification Rules</h4>", unsafe_allow_html=True)

        st.markdown(f"""
        <div style="background:{CARD_COLOR}; border:1px solid {BORDER_COLOR}; border-radius:12px; padding:16px; margin:12px 0;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="color:{TEXT_COLOR}; font-weight:600; font-size:13px;">🔴 CRITICAL Alerts</span>
                <span style="background:{CRITICAL_COLOR}; color:white; padding:2px 8px; border-radius:10px; font-size:11px; font-weight:600;">ACTIVE</span>
            </div>
            <div style="color:{MUTED_TEXT}; font-size:12px; line-height:1.6;">
        ...
        <div style="background:{CARD_COLOR}; border:1px solid {BORDER_COLOR}; border-radius:12px; padding:16px; margin:12px 0;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="color:{TEXT_COLOR}; font-weight:600; font-size:13px;">🟠 HIGH Alerts</span>
                <span style="background:{HIGH_COLOR}; color:white; padding:2px 8px; border-radius:10px; font-size:11px; font-weight:600;">ACTIVE</span>
            </div>
            <div style="color:{MUTED_TEXT}; font-size:12px; line-height:1.6;">
        ...
        <div style="background:{CARD_COLOR}; border:1px solid {BORDER_COLOR}; border-radius:12px; padding:16px; margin:12px 0;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="color:{TEXT_COLOR}; font-weight:600; font-size:13px;">🟡 MEDIUM Alerts</span>
                <span style="background:{MEDIUM_COLOR}; color:white; padding:2px 8px; border-radius:10px; font-size:11px; font-weight:600;">ACTIVE</span>
            </div>
            <div style="color:{MUTED_TEXT}; font-size:12px; line-height:1.6;">
        ...
        <div style="background:{CARD_COLOR}; border:1px solid {BORDER_COLOR}; border-radius:12px; padding:16px; margin:12px 0;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span style="color:{TEXT_COLOR}; font-weight:600; font-size:13px;">🟢 LOW Alerts</span>
                <span style="background:{LOW_COLOR}; color:white; padding:2px 8px; border-radius:10px; font-size:11px; font-weight:600;">ACTIVE</span>
            </div>
            <div style="color:{MUTED_TEXT}; font-size:12px; line-height:1.6;">
        ...
        # Custom rule builder
        st.markdown(f"<h5 style='color:{TEXT_COLOR}; margin-top:16px;'>➕ Custom Rule</h5>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"<h4 style='color:{TEXT_COLOR};'>📊 Notification Analytics</h4>", unsafe_allow_html=True)
        ...
        fig.update_layout(
            plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
            xaxis_title="Notification Count", yaxis_title=None,
            margin=dict(l=20, r=60, t=40, b=20), height=300, showlegend=False
        )
        st.plotly_chart(fig, use_container_width=True)

        # Channel × Severity matrix
        st.markdown(f"<h5 style='color:{TEXT_COLOR}; margin-top:16px;'>Channel × Severity Matrix</h5>", unsafe_allow_html=True)
        ...
        fig.update_layout(
            plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
            xaxis_title="Severity", yaxis_title="Channel",
            margin=dict(l=20, r=80, t=40, b=20), height=350
        )
        st.plotly_chart(fig, use_container_width=True)

        # Unresponded escalation timeline
        st.markdown(f"<h5 style='color:{TEXT_COLOR}; margin-top:16px;'>⏱️ Escalation Timeline</h5>", unsafe_allow_html=True)
        ...
        if len(unresp) > 0:
            ...
            fig.add_hline(y=unresp_hourly.mean(), line_dash="dash", line_color=MUTED_TEXT, 
                         annotation_text=f"Avg: {unresp_hourly.mean():.1f}/hour", annotation_position="top right")
            fig.update_layout(
                plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR, font_color=TEXT_COLOR,
                xaxis_title="Hour of Day", yaxis_title="Unresponded Count",
...
st.divider()
st.markdown(f"""
<div style="text-align:center; color:{MUTED_TEXT}; font-size:12px; padding:20px 0 40px 0;">
    <p style="margin:0 0 8px 0;">🛡️ <b style="color:{TEXT_COLOR};">Buddywise Safety Intelligence Command Center</b></p>
    <p style="margin:0 0 8px 0;">3,000+ simulated alerts across 5 German industrial sites · 6 risk scenarios · ILO: 2.3M work deaths/year</p>
    <p style="margin:0; color:{ACCENT_COLOR}; font-weight:600;">Built by Tanishq Kumar Singh · Working Student (Tech & Ops) Candidate · Berlin, 2026</p>
</div>
""", unsafe_allow_html=True)
